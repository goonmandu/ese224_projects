...
/* Remove_reserver also by Liz */
void Library::remove_reserver(std::string username)
{
    //scan through the internal_rnl vector (which is filled with book information) a struct
    //for each book (struct) check the reservers linked list to find the same username
    //remove the username every time it appears
    for (LikesAndReservers temp : internal_rnl)
    {
        (temp.reservers).remove(username);
    }
}
