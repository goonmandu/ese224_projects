...
bool Teacher::has_borrowed_books_teacher(std::string name)
{
    ...
    //have to search through values aka the usernames in values to find a match for the name given
    int index = values.size();
    for (int i = 0; i < index; i++)
    {
        if (values[i].username == name)
        {
